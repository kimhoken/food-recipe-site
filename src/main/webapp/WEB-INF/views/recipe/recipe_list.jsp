                        ...
                        <aside class="filter-area">
                            ...
                            <div class="filter-section">
                                ...
                                <%-- 삼항 연산자를 써서 코드를 훨씬 깔끔하게 만들기 --%>
                                    <label><input type="radio" name="category" value="상황별추천" ${recipeSearchDTO.category
                                            eq '상황별추천' ? 'checked' : '' }> ⭐ 상황별 추천</label>
                                    <label><input type="radio" name="category" value="한식" ${recipeSearchDTO.category
                                            eq '한식' ? 'checked' : '' }> 🍚 한식</label>
                                    <label><input type="radio" name="category" value="양식" ${recipeSearchDTO.category
                                            eq '양식' ? 'checked' : '' }> 🍝 양식</label>
                                    <label><input type="radio" name="category" value="중식" ${recipeSearchDTO.category
                                            eq '중식' ? 'checked' : '' }> 🍳 중식</label>
                                    <label><input type="radio" name="category" value="일식" ${recipeSearchDTO.category
                                            eq '일식' ? 'checked' : '' }> 🍣 일식</label>
                                    <label><input type="radio" name="category" value="아시안" ${recipeSearchDTO.category
                                            eq '아시안' ? 'checked' : '' }> 🌏 아시안</label>
                                    <label><input type="radio" name="category" value="건강식/다이어트"
                                            ${recipeSearchDTO.category eq '건강식/다이어트' ? 'checked' : '' }> 🥗
                                        건강식/다이어트</label>
                                    <label><input type="radio" name="category" value="초간단요리" ${recipeSearchDTO.category
                                            eq '초간단요리' ? 'checked' : '' }> ⏱️ 초간단요리</label>
                                    <label><input type="radio" name="category" value="디저트" ${recipeSearchDTO.category
                                            eq '디저트' ? 'checked' : '' }> 🍰 디저트</label>
                                    <label><input type="radio" name="category" value="베이킹" ${recipeSearchDTO.category
                                            eq '베이킹' ? 'checked' : '' }> 🍞 베이킹</label>
                                    <label><input type="radio" name="category" value="음료/차" ${recipeSearchDTO.category
                                            eq '음료/차' ? 'checked' : '' }> ☕ 음료/차</label>
                            </div>
                            <hr><br>
                            <div class="filter-section">
                                <h4>조리시간</h4>
                                <%-- 체크박스 상태를 위해 밖에서 미리 변수 세팅하기 --%>
                                    ...
                                    <%-- 세팅된 변수를 EL로 넣어주기만 하면 끝 --%>
                                        <label><input type="checkbox" name="cookTimes" value="10" ${chk10}> 10분
                                            이하</label>
                                        <label><input type="checkbox" name="cookTimes" value="20" ${chk20}>
                                            10~20분</label>
                                        <label><input type="checkbox" name="cookTimes" value="30" ${chk30}>
                                            20~30분</label>
                                        <label><input type="checkbox" name="cookTimes" value="60" ${chk60}>
                                            30~60분</label>
                                        <label><input type="checkbox" name="cookTimes" value="61" ${chk61}> 60분
                                            이상</label>
                            </div>
                                    ...
                                    <div class="paging">
                                        ...
                                        <%-- 1. 반복되는 cookTimes 쿼리 스트링을 밖에서 미리 변수로 만들어두기 --%>
                                            ...
                                            <%-- 2. 이전 버튼 (미리 만든 cookTimesQuery를 concat으로 붙이기) --%>
                                                ...
                                                <c:set var="startP"
                                                    value="${curPage - 1 < 1 ? 1 : (curPage == totalPage and totalPage >= 3 ? totalPage - 2 : curPage - 1)}" />
                                                <c:set var="endP"
                                                    value="${startP + 2 > totalPage ? totalPage : startP + 2}" />
